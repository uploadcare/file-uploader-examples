<script>
  import { page } from '$app/stores';
</script>

<nav class="root">
  <a class="link logo" href="https://uploadcare.com">
    <svg xmlns="http://www.w3.org/2000/svg" focusable="false" viewBox="0 0 18 18" width="30" height="30">
      <circle cx="9" cy="9" r="9" fill="url(#gradient)"></circle>
      <defs>
        <radialGradient id="gradient" cx="0" cy="0" r="1" gradientTransform="rotate(149.216 9.368 7.42) scale(17.5848 20.2492)" gradientUnits="userSpaceOnUse">
          <stop stop-color="#FFC700"></stop>
          <stop offset="1" stop-color="#FFEDAB"></stop>
        </radialGradient>
      </defs>
    </svg>
  </a>

  <ul class="menu">
    <li class="menu-item">
      <a
        class="link menu-link"
        href="/form"
        class:active={$page.url.pathname === '/form'}
      >Real-life form</a>
    </li>

    <li class="menu-item">
      <a
        class="link menu-link"
        href="/minimal"
        class:active={$page.url.pathname === '/minimal'}
      >Minimal uploader</a>
    </li>

    <li class="menu-item">
      <a
        class="link menu-link"
        href="/regular"
        class:active={$page.url.pathname === '/regular'}
      >Regular uploader</a>
    </li>
  </ul>

  <div class="source">
    <span class="source-title">
      Built with Uploadcare Blocks and Svelte
    </span>

    <a class="link" href="https://github.com/uploadcare/blocks-examples/tree/main/examples/angular-uploader">
      <svg xmlns="http://www.w3.org/2000/svg" focusable="false" viewBox="0 0 24 24" width="30" height="30">
        <style>
          path {
            fill: var(--ui-control-text-color);
          }
        </style>
        <path fill="#24292f" stroke="none" d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"></path>
      </svg>
    </a>
  </div>
</nav>

<slot/>

<style lang="scss">
  .root {
    display: grid;
    grid-template-areas:
      "logo source"
      "menu menu";
    grid-template-columns: 1fr auto;
    grid-template-rows: auto 1fr;
    gap: 24px;
    align-items: center;
    padding: 24px 24px 0;
    margin: 0 -24px 24px;
    border-bottom: 1px solid var(--ui-control-border-color-default);
    color: var(--ui-control-text-color);

    @media (width >= 900px) {
      grid-template-areas:
        "logo menu source";
      grid-template-columns: 1fr auto 1fr;
      grid-template-rows: 1fr;
      padding: 0 24px;
    }
  }

  .link:hover {
    opacity: .5;
  }

  .logo {
    grid-area: logo;
  }

  .menu {
    grid-area: menu;
    margin: 0 auto;
    padding: 0;
    list-style: none;
  }

  .menu-item {
    display: inline-block;
    margin-right: 24px;
  }

  .menu-link {
    display: flex;
    padding: 12px 0;
    text-decoration: none;
    color: var(--ui-control-text-color);

    @media (width >= 900px) {
      padding: 24px 0;
    }
  }

  .menu-link.active {
    box-shadow: 0 1px 0 var(--ui-control-border-color-focus);
  }

  .source {
    grid-area: source;
    margin-left: auto;
    display: flex;
    align-items: center;
  }

  .source-title {
    font-size: 14px;
    margin-right: 24px;
    color: var(--ui-text-color);
  }
</style>
